...
const projects = [
  {
    ...
    github: "https://github.com/ebrarkadir/DarkMesh",
    demo: "#",
    translations: {
      tr: {
        title: "DarkMesh",
        shortDescription:
          "Linux tabanlı grafik arayüzlü ağ tarayıcı ve erişim kontrol uygulaması. OpenWRT ile entegre çalışarak cihazları tespit eder ve internetlerini yönetir.",
        fullDescription:
          "DarkMesh, Python ve Tkinter kullanılarak geliştirilen, Linux üzerinde çalışan grafik arayüzlü bir ağ izleme ve kontrol panelidir. Raspberry Pi cihazına yüklenmiş OpenWRT ile entegre şekilde çalışır ve ağdaki cihazları IP, MAC adresi ve üretici bilgileriyle birlikte tespit eder. Kullanıcılar bu cihazların internet erişimlerini doğrudan arayüzden engelleyebilir, tekrar açabilir ve zamanlanmış kurallar tanımlayarak belirli saatlerde otomatik müdahaleler gerçekleştirebilir. Ayrıca cihazlara özel isim ve tip atanabilir, ping testi ve port taraması gibi işlevler ile cihazların durumu analiz edilebilir. Bu proje ile ağ güvenliği, iptables kullanımı, zamanlayıcı sistemleri, Flask API ile uzaktan kontrol, OpenWRT entegrasyonu ve GUI uygulama geliştirme konularında pratik deneyim kazandım.",
      },
      en: {
        title: "DarkMesh",
        shortDescription:
          "A Linux-based GUI network scanner and access control tool integrated with OpenWRT for device detection and internet management.",
        fullDescription:
          "DarkMesh is a graphical network monitoring and control application built with Python and Tkinter for Linux systems. It works in conjunction with a Raspberry Pi device running OpenWRT to detect devices on the local network by their IP, MAC address, and vendor information. Users can block or unblock internet access for specific devices directly from the interface, as well as schedule rules to automatically restrict access at certain times. The system also supports device labeling, ping testing, and port scanning for enhanced analysis. Through this project, I gained hands-on experience in network security, iptables usage, task scheduling, remote control via Flask API, OpenWRT integration, and desktop GUI application development.",
      },
    },
  },
  {
  ...
  },
  {
    ...
    title: "Email Scraper",
    images: [
      "/assets/email-scraper-1.png",
      "/assets/email-scraper-2.png",
    ],
    github: "https://github.com/ebrarkadir/email_scraper",
    demo: "#",
    translations: {
      tr: {
        title: "Email Scraper",
        shortDescription:
          "Belirtilen bir web sitesindeki e-posta adreslerini tespit eden, domain dışına çıkmayan Python tabanlı bir tarayıcı scripti.",
        fullDescription:
          "Email Scraper, kullanıcıdan aldığı başlangıç URL’si ile aynı domain altında bağlantıları tarayarak geçerli e-posta adreslerini toplayan bir Python uygulamasıdır. Genişlik öncelikli (BFS) tarama mantığı ile çalışır, her sayfayı bir kez ziyaret eder ve tarayıcı gibi davranmak için User-Agent header'ı kullanır. HTML içerikleri BeautifulSoup ile parse edilirken, e-posta adresleri regex ile çıkarılır. Kullanıcıya isterse bulunan e-postaları bir `.txt` dosyasına kaydetme seçeneği sunar. Bu proje ile web scraping, HTML parsing, URL yönetimi ve veri çıkarımı gibi konularda pratik deneyim kazandım.",
      },
      en: {
        title: "Email Scraper",
        shortDescription:
          "A Python-based web scraper that identifies email addresses within a given website without going beyond its domain.",
        fullDescription:
          "Email Scraper is a Python script that starts from a user-specified URL and crawls all reachable pages under the same domain to extract valid email addresses. It uses a breadth-first search (BFS) approach with a deque structure, visits each page only once, and mimics a browser by adding a User-Agent header. The script parses HTML content using BeautifulSoup and employs regular expressions to find email patterns. Optionally, users can save the results in a `.txt` file. Through this project, I gained experience in web scraping, HTML parsing, regex-based data extraction, and handling network requests with Python.",
      },
    },
  },
  {
